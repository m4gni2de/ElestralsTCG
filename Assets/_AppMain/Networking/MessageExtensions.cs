...
{
    #region Ownership
    public static bool DidYouSend(this Message message)
    {
        ushort sender = message.GetUShort();
        return (sender == NetworkManager.Instance.Client.Id);
    }
    #endregion
    ...
    #region Generic
    public static Message Add<T>(this Message message, T obj)
    {
        ...
        {
            ushort s = ushort.Parse(obj.ToString());
            message.AddUShort(s);
        }
        if (obj.GetType() == typeof(int))
        {
            int s = int.Parse(obj.ToString());
            message.AddInt(s);
        }
        return message;
    }
